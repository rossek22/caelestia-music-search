...
ColumnLayout {
    ...
    function volumeIcon(volume: real): string {
        if (volume <= 0)
            return "volume_off";
        if (volume < 0.5)
            return "volume_down";
    ...
    StyledText {
        ...
        MouseArea {
            ...
            onClicked: root.searchRequested(Players.active?.trackTitle ?? "")
        }
    }
    ...
    RowLayout {
        ...
        MaterialIcon {
            text: root.volumeIcon(Players.active?.volume ?? 0)
            color: Players.active?.volumeSupported ? Colours.palette.m3onSurfaceVariant : Colours.palette.m3outline
        ...
        StyledSlider {
            ...
            enabled: Players.active?.volumeSupported ?? false
            onInteraction: value => {
                if (Players.active?.volumeSupported)
                    Players.active.volume = Math.max(0, Math.min(1, value));
            }
        }
